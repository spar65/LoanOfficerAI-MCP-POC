...
node test-logging-validation.js
FILE_TEST_RESULT=$?

# 2. Run existing logging tests to ensure compatibility
echo -e "\n${YELLOW}Running original logging tests...${NC}"
...
if [ $ORIGINAL_TEST_RESULT -eq 0 ]; then
  echo -e "${GREEN}✓ Original logging tests passed${NC}"
else
  echo -e "${RED}✗ Original logging tests failed${NC}"
fi
...
if [ -f "server/logs/combined.log" ]; then
  ...
  LOG_SIZE=$(wc -l "server/logs/combined.log" | awk '{print $1}')
...
if [ -f "server/logs/error.log" ]; then
  ...
  LOG_SIZE=$(wc -l "server/logs/error.log" | awk '{print $1}')
  echo -e "  Lines: ${LOG_SIZE}"
else
  echo -e "${RED}✗ error.log does not exist${NC}"
fi

if [ -f "server/logs/mcp.log" ]; then
  echo -e "${GREEN}✓ mcp.log exists${NC}"
  LOG_SIZE=$(wc -l "server/logs/mcp.log" | awk '{print $1}')
  echo -e "  Lines: ${LOG_SIZE}"
else
...
echo -e "\n${BLUE}=======================================${NC}"
echo -e "${YELLOW}Log files are available in: server/logs/${NC}"
echo -e "${BLUE}=======================================${NC}"

# Exit with success if all tests passed
if [ $FILE_TEST_RESULT -eq 0 ] && [ $ORIGINAL_TEST_RESULT -eq 0 ]; then
