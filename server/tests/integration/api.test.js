...
describe.skip('API Integration Tests', () => {
  ...
  describe('Loans API', () => {
    ...
    it('should return loans by status', async () => {
      ...
      if (response.body.length > 0) {
        ...
        // If no active loans are found, this is also acceptable
        console.log('No active loans found in the test data');
      }
    });
  });
  ...
  describe('Equipment API', () => {
    it('should return all equipment', async () => {
      ...
      expect(response.status).toBe(200);
      expect(Array.isArray(response.body)).toBeTruthy();
      // Since this is a mock endpoint, we don't require data to be present
    });
