...
const recommendLoanRestructuring = {
  ...
  description:
    "AI-powered loan restructuring recommendations with multiple scenarios",
  parameters: {
    type: "object",
    properties: {
      loan_id: {
  ...
  handler: async (args, context) => {
    const {
      loan_id,
      restructuring_goal = "improve_cashflow",
      max_term_extension = 24,
    } = args;
    const { requestId } = context;
...
async function generateRestructuringRecommendations(
  ...
  const rankedScenarios = rankRestructuringScenarios(
    scenarios,
    restructuringGoal
  );
...
async function runLoadTest() {
  ...
  console.log(
    `Successful Requests: ${result.totalRequests - result.totalErrors}`
  );
  console.log(
    `Error Rate: ${((result.totalErrors / result.totalRequests) * 100).toFixed(
      2
    )}%`
  );
